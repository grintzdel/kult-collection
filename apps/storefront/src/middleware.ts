...
async function getRegionMap(cacheId: string) {
  ...
  if (
    ...
    try {
      ...
      // Backend indisponible : on retombe sur la région par défaut plutôt que
      // de casser tout le storefront (utile en dev sans backend lancé).
      console.warn(
        "Middleware: impossible de récupérer les régions, fallback sur la région par défaut.",
        error
      )
    }
  }
